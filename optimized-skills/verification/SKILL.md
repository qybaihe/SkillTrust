---
name: verification
description: Route verification tasks to the right specialized verifier and produce a combined report
metadata:
  short-description: Optimized preview for verification
...
Route verification tasks to the right specialized verifier and produce a combined report.

## Use When

- The user explicitly asks for: orchestrate multi-review verification.
...
Allowed by default:
- Read target specs, code summaries, and verifier outputs.
- Delegate to specialized verification Skills when appropriate.
- Write consolidated verification reports.

Ask first:
- Running code or tests.
- Editing source files.
- Reading outside the selected target.

Denied by default:
- Changing code under the name of verification.
- Collecting secrets.
- Using every verifier when only one is needed.
...
Token plan: Move repeated severity schema and report templates into references/config.

Selection plan: Use as orchestrator; delegate to specialized verify-* Skills rather than competing with them.
