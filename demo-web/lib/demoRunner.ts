...
export async function runAnalyze(input: AnalyzeInput): Promise<DemoSummary> {
  ...
  try {
    ...
    } else if ("folderFiles" in input && input.folderFiles) {
      prepared = await prepareFolderUpload(input.folderFiles, input.folderPaths, inputRoot);
    } else {
...
async function prepareFolderUpload(files: File[], relativePaths: string[], inputRoot: string): Promise<DemoSummary["input"]> {
  if (!files.length) {
    throw new Error("No folder files were uploaded.");
  }
  if (files.length > MAX_FOLDER_FILES) {
    throw new Error("Folder contains too many files for the demo limit.");
  }
  const totalBytes = files.reduce((sum, file) => sum + file.size, 0);
  if (totalBytes > MAX_FOLDER_BYTES) {
    throw new Error("Folder upload is larger than the 50 MB demo limit.");
  }

  let folderName = "uploaded-folder";
  for (let index = 0; index < files.length; index += 1) {
    const relativePath = safeRelativeUploadPath(relativePaths[index] || files[index].name);
    if (relativePath.includes("/")) {
      folderName = safeName(relativePath.split("/")[0]);
      break;
    }
  }

  const folderRoot = path.join(inputRoot, folderName);
  await fs.mkdir(folderRoot, { recursive: true });
  for (let index = 0; index < files.length; index += 1) {
    const file = files[index];
    const rawRelativePath = relativePaths[index] || file.name;
    const safeRelativePath = stripTopFolder(safeRelativeUploadPath(rawRelativePath));
    if (!safeRelativePath) {
      continue;
    }
    const target = path.resolve(folderRoot, safeRelativePath);
    if (!target.startsWith(path.resolve(folderRoot))) {
      throw new Error("Folder upload path escapes the demo workspace.");
    }
    await fs.mkdir(path.dirname(target), { recursive: true });
    await fs.writeFile(target, Buffer.from(await file.arrayBuffer()));
  }

  const skillPath = await findSkillTarget(folderRoot);
  return {
    name: folderName,
    kind: "folder",
    skill_path: skillPath
...
async function runSemanticReview(
  ...
  if (!apiKey) {
    ...
    return {
      ...
      model,
      base_url: baseUrl,
      fallback_reason: "No server-side semantic reviewer was available, so the demo used SkillTrust's local draft semantic review."
    };
  }
...
function safeRelativeUploadPath(rawPath: string): string {
  ...
  if (!normalized || normalized === "." || normalized.startsWith("../") || normalized === ".." || path.isAbsolute(normalized)) {
    throw new Error("Folder upload contains an unsafe path.");
  }
  return normalized;
}
